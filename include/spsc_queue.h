...
public:
  void init(SHMSegment *segment, std::size_t capacity)
  {
    ...
    header_ = new (segment->get_address())
        SharedHeader{0, 0xC0FFEE, 0, adjusted, sizeof(T), {}, 0};

    if constexpr (std::is_fundamental<T>::value)
    {
      std::strncpy(header_->schema, std::string(get_type_str<T>()).c_str(),
                   sizeof(header_->schema) - 1);
    }
    else
    {
      std::strncpy(header_->schema, Schema<T>::get_json().c_str(),
                   sizeof(header_->schema) - 1);
    }
